...
(async () => {
  ...
  const topics = [{ topic: "stats-topic" }];
  ...
  kafkaConsumer.on("message", async function (message) {
    ...
    const dateISO8601 = moment(decodedMessage.statDate).toISOString();
    const insertResponse = await pgClient.query({
      text: "INSERT INTO stats(uuid, total, stat_date) VALUES($1, $2, $3)",
      values: [uuidv4(), decodedMessage.total, dateISO8601]
    });
